...
**约束：** `3 ≤ nums.length ≤ 500`，`-1000 ≤ nums[i] ≤ 1000`，`-10⁴ ≤ target ≤ 10⁴`

### 思路

**排序 + 双指针（三数之和的变体）**

1. 与三数之和框架完全相同：排序 → 固定 `nums[i]` → 双指针找另外两个数
2. **区别**：不是找精确解 `sum === 0`，而是维护一个 `closest` 记录当前最接近的和
3. 每次计算 `sum` 后，比较 `|sum - target|` 与 `|closest - target|`，取更小的
4. **提前返回**：如果 `sum === target`，差值为 0，不可能更近，直接返回
5. **指针移动策略**：`sum < target` 则 `left++`，`sum > target` 则 `right--`（和三数之和一样）
  ...
  for (let i = 0; i < nums.length - 2; i++) {
    ...
    while (left < right) {
      ...
      if (sum < target) {
        ...
        return target; // 完美匹配，差值为 0，不可能更近
      }
    }
  }
  return closest;
...
**"最接近"是"精确匹配"的泛化。** 精确匹配是差值为 0 的特殊情况，最接近则是在整个搜索过程中维护最小差值。同一个排序+双指针框架，通过改变比较条件就能解不同问题——这是抽象的力量。框架稳定，变化的只是度量标准。
...
**约束：** `1 ≤ nums.length ≤ 10⁴`，`-10⁴ ≤ nums[i] ≤ 10⁴`，`nums` 已按非递减顺序排序
...
1. 输入是有序的，平方后最大值一定在**两端**（负数绝对值可能很大）
2. 双指针 `left = 0`、`right = n-1`，比较 `|nums[left]|` 和 `|nums[right]|`
3. 取较大的平方值放入结果数组的**末尾**（`pos` 从 `n-1` 递减）
4. 这样从最大值往最小值填充，一次遍历即可得到有序结果

**为什么逆向填充？** 正向填充需要知道最小值在哪，但最小平方值可能在数组中间任意位置。逆向填充直接取两端的最大值，简单明确。
...
function sortedSquares(nums) {
  ...
  while (left <= right) {
    ...
    if (leftSq > rightSq) {
      result[pos] = leftSq; // 左端平方更大
      left++;
    } else {
      result[pos] = rightSq; // 右端平方更大（或相等）
      right--;
    }
    pos--; // 填充位置前移
  }
...
**复杂度：** 时间 O(n)，空间 O(n)（结果数组）

### 核心洞察

**有序性是宝贵的信息，不要丢弃它。** 先平方再排序是 O(n log n)，但利用输入的有序性（平方后的最大值只可能在两端），双指针可以 O(n) 完成。逆向填充是关键技巧——从"确定最大"的方向开始，避免了正向填充时"最小值在中间不确定位置"的复杂性。
...
**约束：** `1 ≤ s.length, t.length ≤ 200`，`s` 和 `t` 只含小写字母和 `#`
...
1. **为什么从右往左？** 退格删除的是前面的字符。从右往左遍历时，遇到 `#` 就知道接下来要跳过几个字符，用计数器就能搞定，无需栈
2. 两个指针 `i` 和 `j` 分别从 `s` 和 `t` 的末尾开始
3. 辅助函数 `getNextValid`：从当前位置往左跳过所有需要退格的字符，返回下一个"有效字符"的位置
4. 比较两个有效字符：不相等返回 `false`，相等则两个指针各左移一步继续

**栈解法（更直观，O(n) 空间）：** 正向遍历，遇字符入栈，遇 `#` 出栈，最后比较两个栈。
...
// 辅助函数：从 index 位置往左，跳过所有被退格删除的字符
// 返回下一个有效字符的位置（可能 < 0 表示没有有效字符了）
...
**退格操作是"反向"的，所以从右往左处理最自然。** 从左到右处理退格需要栈（O(n) 空间），但从右到左只需一个计数器——选择正确的遍历方向可以消除辅助数据结构。**方向的选择本身就是一种算法设计。**
...
给定一个含有 `n` 个正整数的数组和一个正整数 `target`，找出该数组中满足其总和**大于等于** `target` 的长度最小的**连续子数组**，并返回其长度。如果不存在符合条件的子数组，返回 `0`。
...
**进阶：** 能否实现 O(n log n) 的解法？（提示：前缀和 + 二分查找）

### 思路

**滑动窗口（最优解）**

1. 维护窗口 `[left, right]` 和窗口内的累加和 `sum`
2. `right` 不断右移扩展窗口，每次将 `nums[right]` 加入 `sum`
3. 当 `sum >= target` 时，说明当前窗口满足条件：
   - 记录窗口长度 `right - left + 1`，更新最小值
   - 收缩左边界 `left++`，同时从 `sum` 中减去 `nums[left]`
   - 重复收缩直到 `sum < target`——寻找更短的满足条件的窗口
4. `right` 继续右移，重复上述过程

**为什么滑动窗口有效？** 因为数组元素**全为正数**：

- 右移 `right`：`sum` 单调递增
- 左移 `left`：`sum` 单调递减
- 这种单调性保证了 `left` 只需前进不需后退
...
**复杂度：** 时间 O(n)（每个元素最多进出窗口各一次），空间 O(1)

### 核心洞察

**滑动窗口的本质是"单调性剪枝"。** 因为数组元素全为正数，窗口右扩和必增，左缩和必减——这种单调性保证了 `left` 只需前进不需后退，每个元素最多进出窗口各一次，总复杂度 O(n)。**没有单调性就没有滑动窗口**——如果数组包含负数，窗口收缩后和可能反而增加，就不能用此方法了。
