...
**rdk/**: 字节跳动 Meego 渲染引擎专题⭐
- 企业级 Canvas 渲染引擎深度剖析
- Layout-Box 项目完整分析（50,000+行代码规模）
- React + Rust + WebAssembly 混合架构工程实践
- 核心技术：React Reconciler、Flexbox布局、WebGPU硬件加速、虚拟滚动
- 真实案例：字节跳动内部项目 code.byted.org/ies/layout-box
- 查看 rdk/CLAUDE.md 了解详情
...
└── rdk/                                                           # 子模块：字节 Meego 渲染引擎专题
    ├── CLAUDE.md                                                  # L2 模块地图
    └── Layout-Box渲染引擎深度剖析：字节Meego团队的Canvas+Rust实践.md
```
...
```
        ...
        ┌─────────────────────┼─────────────────────┐
        |                     |                     |
   [MVC哲学基础]        [Rust混合架构]        [历史溯源]
        |                     |                     |
  MVC模式解构            Rust企业级架构        Smalltalk简史
  三大框架与MVC               |
                    ┌────────┼────────┬────────┐
                    |        |        |        |
             Rust+Wasm   职责边界  PM真实  架构决策⭐⭐
              实战教程    澄清专题   案例    声明式vs命令式
                             |                 |
                      ┌──────┴──────┐   ┌──────┴──────┐
                      |             |   |             |
                  Canvas调用     决策权力  反思:发明轮子  路径:Preact/Signals
                   (Rust侧)     (谁定什么) (专题9)       (专题10)
```
...
13. ✅ **rdk子模块：Layout-Box渲染引擎深度剖析**（2024-02-09，12000字）⭐
